...
def time_series_split(X: pd.DataFrame, Y: pd.DataFrame, n_splits: int = 5):
    ...
    for train_index, test_index in tscv.split(dates):
        ...
        X_train = X[(X['date'].dt.date >= train_date_start) & (X['date'].dt.date <= train_date_end)]
        Y_train = Y[(Y['date'].dt.date >= train_date_start) & (Y['date'].dt.date <= train_date_end)]
        X_test = X[(X['date'].dt.date >= test_date_start) & (X['date'].dt.date <= test_date_end)]
        Y_test = Y[(Y['date'].dt.date >= test_date_start) & (Y['date'].dt.date <= test_date_end)]

        X_train.drop(columns=['date', 'year', 'month', 'day', 'ticker'], inplace=True)
        Y_train.drop(columns=['date', 'year', 'month', 'day', 'ticker'], inplace=True)
        X_test.drop(columns=['date', 'year', 'month', 'day', 'ticker'], inplace=True)
        Y_test.drop(columns=['date', 'year', 'month', 'day', 'ticker'], inplace=True)
