...
class TechnicalModel(EnsembleObjective):
    ...
    def temporal_train_test_split(X: pd.DataFrame, Y: pd.DataFrame) -> tuple:
        ...
        split_index = X.index[int(len(X.index) * 0.7)]
        xtrain, xtest, ytrain, ytest = (
            X.loc[X.index < split_index],
            X.loc[X.index >= split_index],
            Y.loc[Y.index < split_index],
            Y.loc[Y.index >= split_index],
        )
    ...
    def run_analysis(self, additional_data: None):
        ...
        X = pd.concat(
            [technical_datasets[equity]["X"] for equity in technical_datasets]
        )
        Y = pd.concat(
            [technical_datasets[equity]["Y"] for equity in technical_datasets]
        )
    ...
    def evaluate_baseline(self, technical_datasets: dict) -> dict:
        ...
        for equity in technical_datasets:
            try:
                ...
                baseline_dataset = pd.merge(
                    baseline_assumption, subset_y, left_index=True, right_index=True
                ).dropna()
